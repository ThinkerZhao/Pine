...
{
    return b64_decode_ex(src, len, decsize);
}
...
{
    return b64_encode_ex(src, len);
}

/***************************************************************
 * @brief: base64编解码实现函数
 * *************************************************************/
int bufc = 0;    /* The number of buffers we need */
...
char* b64_buf_realloc(unsigned char* ptr, size_t size)
{
	...
	if (size > bufc * B64_BUFFER_SIZE)
	{
		...
		b64_realloc(ptr, B64_BUFFER_SIZE * bufc);
		if (!buf)
        {
            return NULL;
        } 
...
char *b64_encode_ex(const unsigned char *src, size_t len) 
{
    int i = 0;
    int j = 0;
    char *enc = NULL;
